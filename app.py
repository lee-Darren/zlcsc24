        ...
        for idx, member in enumerate(members):
            with cols[idx % 4]:
                ...
                with st.container(border=True):
                    if st.button("", key=f"member-card-{idx}", use_container_width=True, help=f"點擊查看 {member['name']} 的詳細資訊"):
                        st.session_state.selected_member = member
                        st.rerun()
                    
                    # 在按鈕上方顯示卡片內容
                    st.image(member['img'], use_column_width=True)
                    st.markdown(f"### {member['name']}")
                    st.markdown(f"**{member['role']}**")
