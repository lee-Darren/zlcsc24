...
if page == "首頁介紹":
    ...
    * **社團活動**：交流茶會、聯合迎新、聖誕交換禮物、社內程式競賽與成發。
    ...
    st.markdown(f'<a href="{LINE_COMMUNITY_URL}" target="_blank" class="line-community-btn">💬 點我加入【中崙資研新生提問群】</a>', unsafe_allow_html=True)
    ...
    # ─── 新增：聯絡我們頁面的快速 LINE 社群傳送門 ───
    st.info("💡 溫馨提示：如果想要獲得最即時、最快速的回答，建議直接點擊下方按鈕加入我們的 LINE 新生群發問喔！")
    st.markdown(f'<a href="{LINE_COMMUNITY_URL}" target="_blank" class="line-community-btn">💬 點我秒入【新生 LINE 提問群】</a>', unsafe_allow_html=True)
    st.markdown("---")
